...
template <class C>
void testRandom(long long min_tab, long long max_tab, long long max_memory, int runs, bool epsilon, std::string name_C, std::vector<int> &devs, bool silent);
template <class C>
void testSanity(long long min_tab, long long max_tab, long long max_memory, int runs, bool epsilon, std::string name_C, std::vector<int> &devs, bool silent);
template <class C>
...
template <class C>
void testGeometric(long long min_tab, long long max_tab, long long max_memory, int runs, bool epsilon, bool disjoint, std::string name_C, std::vector<int> &devs, bool silent);
template <class C>
...
template <class C>
void testRandomLowRank(long long min_tab, long long max_tab, long long max_memory, long long min_rank, long long max_rank, int runs, bool epsilon, std::string name_C, std::vector<int> &devs, bool silent);
template <class C>
...
template <class C>
void testImages(std::vector<std::string> &images, long long max_memory, int runs, bool epsilon, std::string name_C, std::vector<int> &devs, bool silent);
template <class C>
void testInteger(long long min_tab, long long max_tab, long long max_memory, int runs, bool epsilon, std::string name_C, std::vector<int> &devs, bool silent, double range);
...
int main(int argc, char *argv[])
{
	...
	if (opt.use_double)
	{
		...
		{
			if (opt.run_sanity)
				testSanity<double>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, false, std::string("double"), opt.devices, opt.silent);
			if (opt.run_sanity_cached)
				testSanityCached<double>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.runs, false, std::string("double"), opt.devices, opt.silent);
			if (opt.run_random)
				testRandom<double>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, false, std::string("double"), opt.devices, opt.silent);
			if (opt.run_random_low_rank)
				testRandomLowRank<double>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.lap_min_rank, opt.lap_max_rank, opt.runs, false, std::string("double"), opt.devices, opt.silent);
			if (opt.run_random_low_rank_cached)
				testRandomLowRankCached<double>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.lap_min_rank, opt.lap_max_rank, opt.runs, false, std::string("double"), opt.devices, opt.silent);
			if (opt.run_geometric)
				testGeometric<double>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, false, false, std::string("double"), opt.devices, opt.silent);
			if (opt.run_geometric_disjoint)
				testGeometric<double>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, false, true, std::string("double"), opt.devices, opt.silent);
			if (opt.run_geometric_cached)
				testGeometricCached<double>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.runs, false, false, std::string("double"), opt.devices, opt.silent);
			if (opt.run_geometric_disjoint_cached)
				testGeometricCached<double>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.runs, false, true, std::string("double"), opt.devices, opt.silent);
			if (opt.images.size() > 1)
				testImages<double>(opt.images, opt.lap_max_memory, opt.runs, false, std::string("double"), opt.devices, opt.silent);
		}
		if (opt.use_epsilon)
		{
			if (opt.run_sanity)
				testSanity<double>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, true, std::string("double"), opt.devices, opt.silent);
			if (opt.run_sanity_cached)
				testSanityCached<double>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.runs, true, std::string("double"), opt.devices, opt.silent);
			if (opt.run_random)
				testRandom<double>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, true, std::string("double"), opt.devices, opt.silent);
			if (opt.run_random_low_rank)
				testRandomLowRank<double>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.lap_min_rank, opt.lap_max_rank, opt.runs, true, std::string("double"), opt.devices, opt.silent);
			if (opt.run_random_low_rank_cached)
				testRandomLowRankCached<double>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.lap_min_rank, opt.lap_max_rank, opt.runs, true, std::string("double"), opt.devices, opt.silent);
			if (opt.run_geometric)
				testGeometric<double>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, true, false, std::string("double"), opt.devices, opt.silent);
			if (opt.run_geometric_disjoint)
				testGeometric<double>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, true, true, std::string("double"), opt.devices, opt.silent);
			if (opt.run_geometric_cached)
				testGeometricCached<double>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.runs, true, false, std::string("double"), opt.devices, opt.silent);
			if (opt.run_geometric_disjoint_cached)
				testGeometricCached<double>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.runs, true, true, std::string("double"), opt.devices, opt.silent);
			if (opt.images.size() > 1)
				testImages<double>(opt.images, opt.lap_max_memory, opt.runs, true, std::string("double"), opt.devices, opt.silent);
		}
	}
	if (opt.use_float)
	{
		...
		{
			if (opt.run_sanity)
				testSanity<float>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, false, std::string("float"), opt.devices, opt.silent);
			if (opt.run_sanity_cached)
				testSanityCached<float>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.runs, false, std::string("float"), opt.devices, opt.silent);
			if (opt.run_random)
				testRandom<float>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, false, std::string("float"), opt.devices, opt.silent);
			if (opt.run_random_low_rank)
				testRandomLowRank<float>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.lap_min_rank, opt.lap_max_rank, opt.runs, false, std::string("float"), opt.devices, opt.silent);
			if (opt.run_random_low_rank_cached)
				testRandomLowRankCached<float>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.lap_min_rank, opt.lap_max_rank, opt.runs, false, std::string("float"), opt.devices, opt.silent);
			if (opt.run_geometric)
				testGeometric<float>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, false, false, std::string("float"), opt.devices, opt.silent);
			if (opt.run_geometric_disjoint)
				testGeometric<float>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, false, true, std::string("float"), opt.devices, opt.silent);
			if (opt.run_geometric_cached)
				testGeometricCached<float>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.runs, false, false, std::string("float"), opt.devices, opt.silent);
			if (opt.run_geometric_disjoint_cached)
				testGeometricCached<float>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.runs, false, true, std::string("float"), opt.devices, opt.silent);
			if (opt.images.size() > 1)
				testImages<float>(opt.images, opt.lap_max_memory, opt.runs, false, std::string("float"), opt.devices, opt.silent);
		}
		if (opt.use_epsilon)
		{
			if (opt.run_sanity)
				testSanity<float>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, true, std::string("float"), opt.devices, opt.silent);
			if (opt.run_sanity_cached)
				testSanityCached<float>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.runs, true, std::string("float"), opt.devices, opt.silent);
			if (opt.run_random)
				testRandom<float>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, true, std::string("float"), opt.devices, opt.silent);
			if (opt.run_random_low_rank)
				testRandomLowRank<float>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.lap_min_rank, opt.lap_max_rank, opt.runs, true, std::string("float"), opt.devices, opt.silent);
			if (opt.run_random_low_rank_cached)
				testRandomLowRankCached<float>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.lap_min_rank, opt.lap_max_rank, opt.runs, true, std::string("float"), opt.devices, opt.silent);
			if (opt.run_geometric)
				testGeometric<float>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, true, false, std::string("float"), opt.devices, opt.silent);
			if (opt.run_geometric_disjoint)
				testGeometric<float>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, true, true, std::string("float"), opt.devices, opt.silent);
			if (opt.run_geometric_cached)
				testGeometricCached<float>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.runs, true, false, std::string("float"), opt.devices, opt.silent);
			if (opt.run_geometric_disjoint_cached)
				testGeometricCached<float>(opt.lap_min_cached, opt.lap_max_cached, opt.lap_max_memory, opt.runs, true, true, std::string("float"), opt.devices, opt.silent);
			if (opt.images.size() > 1)
				testImages<float>(opt.images, opt.lap_max_memory, opt.runs, true, std::string("float"), opt.devices, opt.silent);
		}
	}
	if (opt.run_integer)
	{
		if (opt.use_double)
		{
			...
			if (opt.use_epsilon)
				testInteger<double>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, true, std::string("double"), opt.devices, opt.silent, opt.range);
		}
		if (opt.use_float)
		{
			if (opt.use_single)
				testInteger<float>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, false, std::string("float"), opt.devices, opt.silent, opt.range);
			if (opt.use_epsilon)
				testInteger<float>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, true, std::string("float"), opt.devices, opt.silent, opt.range);
		}
		if (opt.use_int)
		{
			if (opt.use_single)
				testInteger<int>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, false, std::string("int"), opt.devices, opt.silent, opt.range);
			if (opt.use_epsilon)
				testInteger<int>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, true, std::string("int"), opt.devices, opt.silent, opt.range);
		}
		if (opt.use_long)
		{
			printf("\n\nsize: %lld\n", opt.lap_min_tab);
			...
			if (opt.use_epsilon)
				testInteger<long long>(opt.lap_min_tab, opt.lap_max_tab, opt.lap_max_memory, opt.runs, true, std::string("long long"), opt.devices, opt.silent, opt.range);
		}
	}
...
void solveDirectTableCUDA(TP &start_time, int N1, int N2, CF &get_cost_cpu, lap::cuda::Worksharing &ws, long long max_memory, int *rowsol, bool epsilon, bool sequential, bool pinned)
{
	...
	//  	for (int j = 0; j < N2; j++)
	//  	{
	//  		printf("%d\t", (int)hostMatrix.getCost(i, j));
	//  	}
	//  	printf("\n");
	//  }
...
void solveCUDA(TP &start_time, int N1, int N2, CF &get_cost_cpu, STATE *state, lap::cuda::Worksharing &ws, long long max_memory, int *rowsol, bool epsilon, bool silent)
{
	...
	{
		if (!silent)
			lap::displayTime(start_time, "Solver using GPU table", std::cout);
		solveDirectCUDA<SC, TC, CF, STATE, TP>(start_time, N1, N2, get_cost_cpu, state, ws, max_memory, rowsol, epsilon);
	}
	else
	{
		if (!silent)
			lap::displayTime(start_time, "Solver using GPU caching", std::cout);
...
void solveTableCUDA(TP &start_time, int N1, int N2, CF &get_cost_cpu, lap::cuda::Worksharing &ws, long long max_memory, int *rowsol, bool epsilon, bool sequential, bool pinned, bool silent)
{
	...
	else
	{
		...
		{
			if (pinned)
				lap::displayTime(start_time, "Solver using GPU caching of pinned CPU table", std::cout);
			else
				lap::displayTime(start_time, "Solver using GPU caching of pageable CPU table", std::cout);
		}
...
__global__ void getGTCost_sanity_kernel(C *cost, GETCOST getcost, STATE state, int N)
{
			...
			for (int r = 0; r < runs; r++)
			{
				...
				for (long long i = 0; i < rank; i++)
				{
					for (long long j = 0; j < N; j++)
						vec[i * N + j] = distribution(generator);
				}
			...
			for (int r = 0; r < runs; r++)
			{
				...
				for (long long i = 0; i < rank; i++)
				{
					...
					for (long long j = 1; j < N; j++)
						max_vec = std::max(max_vec, vec[i * N + j]);
					min_vec = vec[i * N];
					for (long long j = 1; j < N; j++)
						min_vec = std::min(min_vec, vec[i * N + j]);
				}
	...
	for (unsigned int a = 0; a < images.size() - 1; a++)
	{
		for (unsigned int b = a + 1; b < images.size(); b++)
		{
			...
			for (int r = 0; r < runs; r++)
			{
				...
				int max_devices = 1;
				while ((size_t)max_devices * max_memory / sizeof(C) < (size_t)N1 * (size_t)N2)
					max_devices++;
